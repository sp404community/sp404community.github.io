...
> [!NOTE] Если устройство использует несколько источников питания, приоритет определяется в следующем порядке:
> 1. Блок питания
> 2. USB порт
> 3. Батарея
> Устройство не умеет переключаться между источниками автоматически, необходима перезагрузка.
